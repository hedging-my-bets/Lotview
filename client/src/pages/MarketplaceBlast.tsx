...
function VehicleAccordionItem({
  ...
  const descriptionWithCta = [description, pageDmCta, complianceFooterText].filter(Boolean).join("\n\n");
  ...
  const complianceIssues = useMemo(
    ...
    [title, descriptionWithCta, includeComplianceFooter, includePageDmCta, pageDmLink]
  );

  const qualitySignals = useMemo(
    () => buildQualitySignals(title, descriptionWithCta, vehicle.images?.length || 0),
    [title, descriptionWithCta, vehicle.images?.length]
  );
              ...
              <div className="space-y-4">
                ...
                <div>
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <CopyButton text={descriptionWithCta} label="Copy" />
                  </div>
...
export default function MarketplaceBlast() {
  ...
  const markPostedMutation = useMutation({
    mutationFn: async ({ vehicleId, listingUrl }: { vehicleId: number; listingUrl: string }) => {
      const token = localStorage.getItem('auth_token');
