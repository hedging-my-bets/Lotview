...
function MessageThread({
  ...
  const lastInboundAt = useMemo(() => {
    const inbound = messages.filter((msg) => msg.isFromCustomer);
    return inbound.length > 0 ? inbound[inbound.length - 1].sentAt : null;
  }, [messages]);
  ...
  const canSend = messagingWindowOpen;

  const handleSend = () => {
    if (newMessage.trim() && !isSending && canSend) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="p-4 border-t border-border bg-card">
        {!messagingWindowOpen && (
          <div className="max-w-3xl mx-auto mb-3 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-700">
            Messaging window closed. You can reply once the customer messages again.
          </div>
        )}
        <div className="flex gap-2 max-w-3xl mx-auto">
          ...
          <Button
            onClick={handleSend}
            disabled={!newMessage.trim() || isSending || !canSend}
