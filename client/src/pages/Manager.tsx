...
export default function Manager() {
  ...
  useEffect(() => {
    if (activeManagerTab === 'settings' && user) {
      loadMetaSettings();
    }
  }, [activeManagerTab, user]);
  ...
  const loadMetaSettings = async () => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const [accounts, pages, rooftopList, dealershipSettings] = await Promise.all([
        apiGet<any>('/api/facebook/business/accounts', { 'Authorization': `Bearer ${token}` }).catch(() => []),
        apiGet<any>('/api/facebook/connected-pages', { 'Authorization': `Bearer ${token}` }).catch(() => []),
        apiGet<any>('/api/rooftops', { 'Authorization': `Bearer ${token}` }).catch(() => []),
        apiGet<any>('/api/manager/dealership-settings', { 'Authorization': `Bearer ${token}` }).catch(() => null),
      ]);
  ...
  const handleConnectBusiness = async () => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const result = await apiPost<any>('/api/facebook/business/oauth/start', {}, {
        'Authorization': `Bearer ${token}`
      });
  ...
  const handleSaveDealershipDmLink = async () => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      const result = await apiPost<any>('/api/manager/dealership-settings', {
  ...
  const handleSavePageMeta = async (pageId: string) => {
    try {
      ...
      await apiPatch(`/api/facebook/pages/${pageId}/metadata`, {
  ...
  const handleSaveSystemUserToken = async (businessId: string) => {
    try {
      ...
      await apiPost('/api/facebook/business/system-user-token', {
        businessId,
  ...
  const handleCreateRooftop = async () => {
    ...
    try {
      const token = localStorage.getItem('auth_token');
      await apiPost('/api/rooftops', { name: newRooftopName.trim() }, {
        'Authorization': `Bearer ${token}`
      });
      setNewRooftopName("");
              ...
              {activeManagerTab === 'settings' && (
                <div data-testid="tab-content-settings">
                  ...
                  <div className="mt-10 space-y-6">
                    <div className="flex items-center justify-between">
                      <div>
                        <h3 className="text-lg font-semibold">Meta Business Connect</h3>
                        <p className="text-sm text-muted-foreground">
                          Connect your Business, configure webhooks, and route Page DMs to rooftops.
                        </p>
                      </div>
                      <Button
                        variant="outline"
                        onClick={loadMetaSettings}
                        disabled={isLoadingMetaSettings}
                      ...
                      <CardContent className="space-y-4">
                        <div className="flex flex-col md:flex-row md:items-center gap-3">
                          <Button
                            ...
                            data-testid="button-connect-business"
                          >
                        ...
                        {businessAccounts.length === 0 ? (
                          <div className="text-sm text-muted-foreground">
                            No Business accounts connected yet.
                          </div>
                        ) : (
                          <div className="space-y-4">
                                ...
                                <div key={account.id} className="border rounded-lg p-4 space-y-3">
                                  ...
                                  <div className="flex items-center justify-between">
                                    ...
                                    <Button
                                      ...
                                      data-testid={`save-system-token-${account.businessId}`}
                                    >
                                      <Save className="w-4 h-4 mr-2" />
                                      Save Token
                                    </Button>
                        ...
                        <div>
                          ...
                          <div className="flex flex-col md:flex-row gap-2 mt-2">
                            <Input
                              ...
                              data-testid="input-dealership-dm-link"
                            />
                            <Button
                              onClick={handleSaveDealershipDmLink}
                              disabled={isSavingDealershipDmLink}
                              data-testid="button-save-dealership-dm-link"
                            >
                              <Save className="w-4 h-4 mr-2" />
                              Save
                            </Button>
                              ...
                              <div key={page.id} className="border rounded-lg p-4 space-y-3">
                                ...
                                <div className="flex justify-end">
                                  <Button
                                    variant="outline"
                                    size="sm"
                                    onClick={() => handleSavePageMeta(page.pageId)}
                                    data-testid={`save-page-meta-${page.pageId}`}
                                  >
                                    <Save className="w-4 h-4 mr-2" />
                                    Save Page
                                  </Button>
                    ...
                    <Card>
                      <CardHeader>
                        ...
                        <CardDescription>
                          Create rooftops to route Page conversations and reporting.
                        </CardDescription>
                      </CardHeader>
                      <CardContent className="space-y-3">
                        <div className="flex flex-col md:flex-row gap-2">
                          <Input
                            ...
                            data-testid="input-rooftop-name"
                          />
                          <Button
                            onClick={handleCreateRooftop}
                            disabled={isCreatingRooftop || !newRooftopName.trim()}
                            data-testid="button-create-rooftop"
                          >
                            <Plus className="w-4 h-4 mr-2" />
                            Add Rooftop
                          </Button>
