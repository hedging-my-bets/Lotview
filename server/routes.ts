...
const oauthStateStore = new Map<string, { userId: number; accountId?: number; dealershipId: number; expiresAt: number; flow?: 'personal' | 'business' }>();
  ...
  app.post("/api/messenger-conversations/:id/reply", authMiddleware, requireRole("manager", "admin", "master", "super_admin"), async (req, res) => {
    try {
      ...
      if (!lastInboundAt || (Date.now() - lastInboundAt.getTime()) > 24 * 60 * 60 * 1000) {
        return res.status(403).json({ error: "Messaging window closed (outside 24-hour window)" });
      }
      ...
      await storage.logAuditAction({
        ...
        resource: 'messenger_conversation',
...
Format your response in clear sections with actionable recommendations.`;
  ...
  app.post("/api/facebook/queue", authMiddleware, requireRole("salesperson"), async (req, res) => {
    try {
      ...
      const vehicle = await storage.getVehicleById(validated.data.vehicleId, dealershipId);
      if (!vehicle) {
        return res.status(404).json({ error: "Vehicle not found" });
      }
      const vin = vehicle.vin?.trim();
      if (!vin) {
        return res.status(400).json({ error: "Vehicle VIN is required to queue for posting" });
      }
  ...
  // Start Business OAuth session (dealer admin connects Pages via Business Login)
  app.post("/api/facebook/business/oauth/start", authMiddleware, requireRole("manager", "admin", "master", "super_admin"), async (req, res) => {
    try {
      const authReq = req as AuthRequest;
      const userId = authReq.user!.id;
      const dealershipId = req.dealershipId!;
      ...
      res.status(500).json({ error: "Failed to start Business OAuth flow" });
    }
  });

  // Save System User token for Business (admin-controlled)
  app.post("/api/facebook/business/system-user-token", authMiddleware, requireRole("manager", "admin", "master", "super_admin"), async (req, res) => {
    try {
      const dealershipId = req.dealershipId!;
      const userId = req.user!.id;
      const { businessId, systemUserId, systemUserToken, tokenExpiresAt } = req.body;

      if (!businessId || !systemUserToken) {
        return res.status(400).json({ error: "businessId and systemUserToken are required" });
      }

      const existing = await storage.getFacebookBusinessAccountByBusinessId(dealershipId, businessId);
      if (!existing) {
        return res.status(404).json({ error: "Business account not found. Connect business first." });
      }

      const updated = await storage.updateFacebookBusinessAccount(existing.id, dealershipId, {
        systemUserId: systemUserId || existing.systemUserId,
        systemUserToken,
      ...
      logError('Error saving system user token:', error instanceof Error ? error : new Error(String(error)), { route: 'api-facebook-business-system-user-token' });
      res.status(500).json({ error: "Failed to save system user token" });
    }
  });
    ...
    try {
      ...
      if (flow === 'business') {
        ...
        for (const business of businesses) {
          const existing = await storage.getFacebookBusinessAccountByBusinessId(dealershipId, business.id);
          if (existing) {
            await storage.updateFacebookBusinessAccount(existing.id, dealershipId, {
              businessName: business.name,
              systemUserToken: longLivedToken.accessToken,
              tokenExpiresAt: expiresAt,
              isActive: true,
              connectedById: userId
            });
          } else {
            await storage.createFacebookBusinessAccount({
              dealershipId,
        ...
        for (const page of pages) {
          const existingPage = await storage.getFacebookPageByPageId(page.id);
          if (existingPage) {
            await storage.updateFacebookPage(existingPage.id, {
              pageName: page.name,
        ...
        return res.send(`
          <html>
            ...
            <body style="font-family: system-ui; text-align: center; padding: 50px;">
              <h1>✅ Facebook Business Connected</h1>
              <p>Pages are now connected to LotView. You can close this window.</p>
              <button onclick="window.close()">Close</button>
            </body>
          </html>
        `);
  ...
  // Update Facebook page metadata (DM link + rooftop routing)
  app.patch("/api/facebook/pages/:pageId/metadata", authMiddleware, requireRole("manager", "admin", "master", "super_admin"), async (req, res) => {
    try {
      const dealershipId = req.dealershipId!;
      const pageId = req.params.pageId;
      const { pageDmLink, rooftopId } = req.body;

      const page = await storage.getFacebookPageByPageId(pageId);
      if (!page || page.dealershipId !== dealershipId) {
        return res.status(404).json({ error: "Page not found" });
      }

      const rooftopIdValue = rooftopId ? parseInt(rooftopId) : null;
      if (rooftopIdValue) {
        const rooftops = await storage.getRooftops(dealershipId);
        if (!rooftops.some((r) => r.id === rooftopIdValue)) {
          return res.status(400).json({ error: "Invalid rooftop" });
        }
      }

      const updated = await storage.updateFacebookPage(page.id, {
        pageDmLink: typeof pageDmLink === 'string' ? pageDmLink.trim() : null,
        rooftopId: rooftopIdValue || null
      });

      res.json(updated);
    } catch (error) {
      logError('Error updating Facebook page metadata:', error instanceof Error ? error : new Error(String(error)), { route: 'api-facebook-pages-metadata' });
      res.status(500).json({ error: "Failed to update page metadata" });
    }
  });

  // Get connected Facebook Business accounts (safe fields)
  app.get("/api/facebook/business/accounts", authMiddleware, requireRole("manager", "admin", "master", "super_admin"), async (req, res) => {
    try {
      const dealershipId = req.dealershipId!;
      const accounts = await storage.getFacebookBusinessAccounts(dealershipId);
      const safeAccounts = accounts.map(({ systemUserToken, ...account }) => account);
      res.json(safeAccounts);
    } catch (error) {
      logError('Error fetching Facebook business accounts:', error instanceof Error ? error : new Error(String(error)), { route: 'api-facebook-business-accounts' });
  ...
  app.post("/api/facebook/post/:queueId", authMiddleware, requireRole("salesperson"), async (req, res) => {
    try {
      if (!marketplacePublisher.isEnabled()) {
        return res.status(403).json({ error: "Marketplace auto-posting requires Marketplace Partner approval. Use Marketplace Blast copilot workflow." });
      }
      ...
      try {
        ...
        res.json({ success: true, postId: listingId });
      } catch (error) {
  ...
  app.post("/api/manager/market-pricing", authMiddleware, requireRole("manager"), async (req: AuthRequest, res) => {
    try {
      ...
      const searchRadiusKm = radiusKm || userSettings?.defaultRadiusKm || 500;
  ...
  app.post("/api/manager/enhanced-market-analysis", authMiddleware, requireRole("manager"), async (req, res) => {
    try {
      const { years, make, model, trims, mileage, radiusKm, postalCode, targetPrice, skipAggregation, skipPriceHistory, skipAiInsights } = req.body;
      ...
      const searchRadiusKm = radiusKm || settings?.defaultRadiusKm || 500;
      ...
      const result = await enhancedMarketAnalysis.analyze({
        ...
        skipAggregation: skipAggregation === true,
        skipPriceHistory: skipPriceHistory === true,
        skipAiInsights: skipAiInsights === true
      });
  ...
  app.post("/api/manager/live-market-stats", authMiddleware, requireRole("manager"), async (req, res) => {
    try {
      ...
      const result = await service.getLiveMarketStats({
        ...
        radiusKm: radiusKm ? parseInt(radiusKm) : (settings?.defaultRadiusKm || 500),
        maxResults: 100,
        dealershipId
      });
  ...
  // Rooftops
  app.get("/api/rooftops", authMiddleware, requireRole("manager", "admin", "master", "super_admin"), async (req, res) => {
    try {
      const dealershipId = req.dealershipId!;
      const rooftops = await storage.getRooftops(dealershipId);
      res.json(rooftops);
    } catch (error) {
      logError('Error fetching rooftops:', error instanceof Error ? error : new Error(String(error)), { route: 'api-rooftops' });
      res.status(500).json({ error: "Failed to fetch rooftops" });
    }
  });

  app.post("/api/rooftops", authMiddleware, requireRole("manager", "admin", "master", "super_admin"), async (req, res) => {
    try {
      const dealershipId = req.dealershipId!;
      const { name, slug, address, city, province, postalCode, phone } = req.body;

      if (!name || typeof name !== 'string') {
        return res.status(400).json({ error: "Rooftop name is required" });
      }

      const slugValue = typeof slug === 'string' && slug.trim().length > 0
        ? slug.trim()
        : name.trim().toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '');

      const rooftop = await storage.createRooftop({
        dealershipId,
        name: name.trim(),
        slug: slugValue,
      ...
      res.status(201).json(rooftop);
    } catch (error) {
      logError('Error creating rooftop:', error instanceof Error ? error : new Error(String(error)), { route: 'api-rooftops' });
      res.status(500).json({ error: "Failed to create rooftop" });
    }
  });

  // Rooftop memberships
  app.get("/api/rooftops/:id/memberships", authMiddleware, requireRole("manager", "admin", "master", "super_admin"), async (req, res) => {
    try {
      const dealershipId = req.dealershipId!;
      const rooftopId = parseInt(req.params.id);
      if (isNaN(rooftopId)) {
        return res.status(400).json({ error: "Invalid rooftop ID" });
      }

      const rooftops = await storage.getRooftops(dealershipId);
      if (!rooftops.some((rooftop) => rooftop.id === rooftopId)) {
        return res.status(404).json({ error: "Rooftop not found" });
      }

      const memberships = await storage.getRooftopMemberships(dealershipId, rooftopId);
      res.json(memberships);
    } catch (error) {
      logError('Error fetching rooftop memberships:', error instanceof Error ? error : new Error(String(error)), { route: 'api-rooftops-id-memberships' });
      res.status(500).json({ error: "Failed to fetch rooftop memberships" });
    }
  });

  app.post("/api/rooftops/:id/memberships", authMiddleware, requireRole("manager", "admin", "master", "super_admin"), async (req, res) => {
    try {
      const dealershipId = req.dealershipId!;
      const rooftopId = parseInt(req.params.id);
      if (isNaN(rooftopId)) {
      ...
      const { userId, canPost, role } = req.body;
      ...
      const user = await storage.getUserById(userId, dealershipId);
      if (!user) {
        return res.status(404).json({ error: "User not found" });
      }

      const allowedRoles = new Set(['rep', 'manager', 'admin']);
      const normalizedRole = typeof role === 'string' && allowedRoles.has(role) ? role : 'rep';
      const membership = await storage.upsertRooftopMembership({
        dealershipId,
        rooftopId,
        userId,
        role: normalizedRole,
        canPost: typeof canPost === 'boolean' ? canPost : false
      });
      ...
      logError('Error updating rooftop membership:', error instanceof Error ? error : new Error(String(error)), { route: 'api-rooftops-id-memberships' });
      res.status(500).json({ error: "Failed to update rooftop membership" });
    }
  });

  // Get dealership-level settings (manager)
  app.get("/api/manager/dealership-settings", authMiddleware, requireRole("manager", "admin", "master", "super_admin"), async (req, res) => {
    try {
      const dealershipId = req.dealershipId!;
      const dealership = await storage.getDealership(dealershipId);
      if (!dealership) {
      ...
      res.json({ pageDmLink: dealership.pageDmLink || null });
    } catch (error) {
      logError('Error fetching dealership settings:', error instanceof Error ? error : new Error(String(error)), { route: 'api-manager-dealership-settings' });
      res.status(500).json({ error: "Failed to fetch dealership settings" });
    }
  });

  // Update dealership-level settings (manager)
  app.post("/api/manager/dealership-settings", authMiddleware, requireRole("manager", "admin", "master", "super_admin"), async (req, res) => {
    try {
      const dealershipId = req.dealershipId!;
      const { pageDmLink } = req.body;

      const updated = await storage.updateDealership(dealershipId, {
        pageDmLink: typeof pageDmLink === 'string' ? pageDmLink.trim() : null
      });

      res.json({ pageDmLink: updated?.pageDmLink || null });
    } catch (error) {
      logError('Error updating dealership settings:', error instanceof Error ? error : new Error(String(error)), { route: 'api-manager-dealership-settings' });
      res.status(500).json({ error: "Failed to update dealership settings" });
    }
  });

  // Get dealership settings (read-only for authenticated users)
  app.get("/api/dealership/settings", authMiddleware, async (req, res) => {
    try {
      const dealershipId = req.dealershipId!;
      const dealership = await storage.getDealership(dealershipId);
      if (!dealership) {
      ...
      logError('Error fetching dealership settings (read-only):', error instanceof Error ? error : new Error(String(error)), { route: 'api-dealership-settings' });
      res.status(500).json({ error: "Failed to fetch dealership settings" });
    }
  });
    ...
    try {
      ...
      if (existing) {
        const updated = await storage.updateManagerSettings(userId, dealershipId, {
          postalCode,
          defaultRadiusKm: defaultRadiusKm || 500,
  ...
  // ===== META WEBHOOKS (Messenger/IG) =====
  app.get("/webhooks/meta", (req, res) => {
    const mode = req.query["hub.mode"];
    const token = req.query["hub.verify_token"];
    const challenge = req.query["hub.challenge"];

    if (mode === "subscribe" && token && token === facebookWebhookVerifyToken) {
    ...
    try {
      ...
      for (const entry of body.entry) {
        ...
        for (const event of entry.messaging) {
          ...
          } else if (!conversation.rooftopId && page.rooftopId) {
            await storage.updateMessengerConversation(conversation.id, dealershipId, {
              rooftopId: page.rooftopId
            });
            conversation = { ...conversation, rooftopId: page.rooftopId };
          }

          const targetRooftopId = conversation.rooftopId ?? null;
          if (targetRooftopId && !conversation.assignedToUserId) {
            const assignee = await storage.pickAssigneeForRooftop(dealershipId, targetRooftopId);
            if (assignee) {
              await storage.updateConversationAssignment(dealershipId, conversation.id, assignee.id);
              conversation = { ...conversation, assignedToUserId: assignee.id };
            }
          }
      ...
      logError('Error processing Meta webhook:', error instanceof Error ? error : new Error(String(error)), { route: 'webhooks-meta' });
      res.sendStatus(500);
    }
  });
