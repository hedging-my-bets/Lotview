...
export function maximizeImageUrl(url: string): string {
  ...
  if (/autotradercdn\.ca/i.test(url)) {
    ...
    maximized = url.replace(/\/\d{3,4}\//g, '/2048/').replace(/sz=\d+/gi, 'sz=2048').replace(/size=\d+/gi, 'size=2048');
  } else if (/dealerinspire/i.test(url)) {
    maximized = url.replace(/\/thumb\//g, '/original/').replace(/\/small\//g, '/original/').replace(/\/medium\//g, '/original/');
  } else if (/ddclstatic|dealer\.com/i.test(url)) {
    maximized = url.replace(/_\d+x\d+\./g, '_2048x1536.');
  } else if (/spincar\.com/i.test(url)) {
    maximized = url.replace(/\/\d+\//g, '/2048/');
  } else if (/cloudinary\.com/i.test(url)) {
    maximized = url.replace(/\/w_\d+/g, '/w_2048').replace(/\/h_\d+/g, '/h_1536').replace(/\/q_\d+/g, '/q_90');
  } else {
    maximized = url.replace(/-\d+x\d+\./g, '-2048x1536.').replace(/_\d+x\d+\./g, '_2048x1536.');
  }
