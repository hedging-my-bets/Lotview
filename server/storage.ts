...
import { 
  ...
  type InsertFacebookBusinessAccount,
  type AdTemplate,
  type InsertAdTemplate,
  type ListingBundle,
  type InsertListingBundle,
  type ListingInstance,
  type InsertListingInstance,
  type PostingQueue,
...
export interface IStorage {
  ...
  // Rooftops
  getRooftops(dealershipId: number): Promise<Rooftop[]>;
  createRooftop(rooftop: InsertRooftop): Promise<Rooftop>;
  updateRooftop(id: number, dealershipId: number, rooftop: Partial<InsertRooftop>): Promise<Rooftop | undefined>;
  ...
  getMessengerConversationByConversationId(dealershipId: number, conversationId: string): Promise<MessengerConversation | undefined>;
  ...
  getLastInboundMessageAt(dealershipId: number, conversationId: number): Promise<Date | null>;
  ...
  getMessengerMessageByFacebookId(dealershipId: number, facebookMessageId: string): Promise<MessengerMessage | undefined>;
  ...
  getFacebookBusinessAccounts(dealershipId: number): Promise<FacebookBusinessAccount[]>;
  getFacebookBusinessAccountByBusinessId(dealershipId: number, businessId: string): Promise<FacebookBusinessAccount | undefined>;
  createFacebookBusinessAccount(account: InsertFacebookBusinessAccount): Promise<FacebookBusinessAccount>;
  updateFacebookBusinessAccount(id: number, dealershipId: number, updates: Partial<InsertFacebookBusinessAccount>): Promise<FacebookBusinessAccount | undefined>;
...
export class DatabaseStorage implements IStorage {
  ...
  // Rooftops
  async getRooftops(dealershipId: number): Promise<Rooftop[]> {
    return await db.select().from(rooftops).where(eq(rooftops.dealershipId, dealershipId)).orderBy(rooftops.name);
  }

  async createRooftop(rooftop: InsertRooftop): Promise<Rooftop> {
    if (!rooftop.dealershipId) {
      throw new Error('dealershipId is required when creating a rooftop');
    }
    const result = await db.insert(rooftops).values(rooftop).returning();
    return result[0];
  }

  async updateRooftop(id: number, dealershipId: number, rooftop: Partial<InsertRooftop>): Promise<Rooftop | undefined> {
    const result = await db.update(rooftops)
      .set({ ...rooftop, updatedAt: new Date() })
      .where(and(
        eq(rooftops.id, id),
        eq(rooftops.dealershipId, dealershipId)
      ))
      .returning();
    return result[0];
  }

  async getRooftopMemberships(dealershipId: number, rooftopId: number): Promise<(RooftopMembership & { userName: string; userRole: string })[]> {
    const rows = await db.select()
      .from(rooftopMemberships)
      .innerJoin(users, eq(rooftopMemberships.userId, users.id))
      .where(and(
    ...
    return rows.map(row => ({
      ...row.rooftop_memberships,
      userName: row.users.name,
      userRole: row.users.role
    }));
  }

  async upsertRooftopMembership(params: { dealershipId: number; rooftopId: number; userId: number; role?: string; canPost?: boolean }): Promise<RooftopMembership> {
    const { dealershipId, rooftopId, userId, role, canPost } = params;
    ...
    if (existing[0]) {
      const result = await db.update(rooftopMemberships)
        .set({
          role: role || existing[0].role,
          canPost: typeof canPost === 'boolean' ? canPost : existing[0].canPost
        })
        .where(eq(rooftopMemberships.id, existing[0].id))
        .returning();
      return result[0];
    }

    const result = await db.insert(rooftopMemberships)
      .values({
        dealershipId,
        rooftopId,
        userId,
  ...
  async getAssignableUsersForRooftop(dealershipId: number, rooftopId: number): Promise<{ id: number; name: string }[]> {
  ...
  async pickAssigneeForRooftop(dealershipId: number, rooftopId: number): Promise<{ id: number; name: string } | null> {
    ...
    const rows = await db.select({
      id: users.id,
      name: users.name,
      assignedCount: assignedCount.as('assigned_count')
    })
      .from(rooftopMemberships)
      .innerJoin(users, eq(rooftopMemberships.userId, users.id))
      .leftJoin(messengerConversations, and(
        eq(messengerConversations.assignedToUserId, users.id),
        eq(messengerConversations.dealershipId, dealershipId),
        eq(messengerConversations.rooftopId, rooftopId),
        eq(messengerConversations.status, 'active')
      ))
      .where(and(
        eq(rooftopMemberships.dealershipId, dealershipId),
        eq(rooftopMemberships.rooftopId, rooftopId),
      ...
      .orderBy(asc(assignedCount), asc(users.name))
      .limit(1);
  ...
  async getMessengerConversations(dealershipId: number, userId?: number, userRole?: string): Promise<(MessengerConversation & { ownerName?: string; assignedTo?: { id: number; name: string } })[]> {
    ...
    let assignedConversationIds: number[] = [];
    ...
    if (userRole === 'salesperson' && userId) {
      ...
      const assignedRows = await db.select({ conversationId: conversationAssignments.conversationId })
        .from(conversationAssignments)
        .where(and(
          eq(conversationAssignments.dealershipId, dealershipId),
          eq(conversationAssignments.assignedToUserId, userId)
        ));
      assignedConversationIds = assignedRows.map(row => row.conversationId);

      if (accountIds.length === 0 && assignedConversationIds.length === 0) {
        return [];
      }
    }

    const conditions: SQL[] = [eq(messengerConversations.dealershipId, dealershipId)];
    if (userRole === 'salesperson' && userId) {
      const accessConditions: SQL[] = [];
      if (accountIds.length > 0) {
        accessConditions.push(inArray(messengerConversations.facebookAccountId, accountIds));
      }
      accessConditions.push(eq(messengerConversations.assignedToUserId, userId));
    ...
    const rows = await db.select()
      ...
      .leftJoin(facebookAccounts, eq(messengerConversations.facebookAccountId, facebookAccounts.id))
      .leftJoin(users, eq(facebookAccounts.userId, users.id))
    ...
    const assignments = await db.select({
      ...
      .where(and(
        eq(conversationAssignments.dealershipId, dealershipId),
        inArray(conversationAssignments.conversationId, conversationIds)
      ));

    const assignmentMap = new Map<number, number>();
    for (const assignment of assignments) {
      assignmentMap.set(assignment.conversationId, assignment.assignedToUserId);
    }

    const assignedIds = new Set<number>();
    for (const conversation of conversations) {
      const assignedId = conversation.assignedToUserId || assignmentMap.get(conversation.id);
      if (assignedId) {
        assignedIds.add(assignedId);
      }
    }

    const assignedUsersMap = new Map<number, { id: number; name: string }>();
    if (assignedIds.size > 0) {
      const assignedUsers = await db.select({
        ...
        .from(users)
        .where(inArray(users.id, Array.from(assignedIds)));
      for (const user of assignedUsers) {
        assignedUsersMap.set(user.id, { id: user.id, name: user.name });
      }
    }
    ...
    return conversations.map(conversation => {
      ...
      return {
        ...conversation,
        assignedTo: assignedId ? assignedUsersMap.get(assignedId) : undefined
      };
    });
  }
  ...
  async getMessengerConversationByConversationId(dealershipId: number, conversationId: string): Promise<MessengerConversation | undefined> {
    const result = await db.select()
      .from(messengerConversations)
      .where(and(
        eq(messengerConversations.dealershipId, dealershipId),
        eq(messengerConversations.conversationId, conversationId)
      ))
      .limit(1);
    return result[0];
  }
  ...
  async getMessengerConversationById(id: number, dealershipId: number): Promise<(MessengerConversation & { pageAccessToken: string }) | undefined> {
    const result = await db.select()
      .from(messengerConversations)
      .leftJoin(facebookAccounts, eq(messengerConversations.facebookAccountId, facebookAccounts.id))
      .leftJoin(facebookPages, eq(messengerConversations.pageId, facebookPages.pageId))
      .where(and(
        eq(messengerConversations.id, id),
        eq(messengerConversations.dealershipId, dealershipId)
      ))
    ...
    const pageAccessToken = result[0]?.facebook_accounts?.accessToken || result[0]?.facebook_pages?.accessToken;
    if (!result[0] || !pageAccessToken) {
      return undefined;
    }
  ...
  async getLastInboundMessageAt(dealershipId: number, conversationId: number): Promise<Date | null> {
    const result = await db.select({ sentAt: messengerMessages.sentAt })
      .from(messengerMessages)
      .where(and(
        eq(messengerMessages.dealershipId, dealershipId),
        eq(messengerMessages.conversationId, conversationId),
        eq(messengerMessages.isFromCustomer, true)
      ))
  ...
  async getMessengerMessageByFacebookId(dealershipId: number, facebookMessageId: string): Promise<MessengerMessage | undefined> {
    const result = await db.select()
      .from(messengerMessages)
      .where(and(
        eq(messengerMessages.dealershipId, dealershipId),
        eq(messengerMessages.facebookMessageId, facebookMessageId)
      ))
      .limit(1);
    return result[0];
  }
  ...
  async updateConversationAssignment(dealershipId: number, conversationId: number, assignedToUserId: number, assignedByUserId?: number): Promise<ConversationAssignment | undefined> {
    ...
    if (existing) {
      ...
      const assignment = await this.assignConversation({
        dealershipId,
        conversationId,
        assignedToUserId,
        assignedByUserId
  ...
  // ====== FACEBOOK BUSINESS ACCOUNTS ======
  async getFacebookBusinessAccounts(dealershipId: number): Promise<FacebookBusinessAccount[]> {
    return await db.select().from(facebookBusinessAccounts).where(
      eq(facebookBusinessAccounts.dealershipId, dealershipId)
    );
  }

  async getFacebookBusinessAccountByBusinessId(dealershipId: number, businessId: string): Promise<FacebookBusinessAccount | undefined> {
    const result = await db.select().from(facebookBusinessAccounts).where(and(
      eq(facebookBusinessAccounts.dealershipId, dealershipId),
      eq(facebookBusinessAccounts.businessId, businessId)
    )).limit(1);
    return result[0];
  }

  async createFacebookBusinessAccount(account: InsertFacebookBusinessAccount): Promise<FacebookBusinessAccount> {
    if (!account.dealershipId) {
      throw new Error('dealershipId is required when creating a Facebook business account');
    }
    const result = await db.insert(facebookBusinessAccounts).values(account).returning();
    return result[0];
  }

  async updateFacebookBusinessAccount(id: number, dealershipId: number, updates: Partial<InsertFacebookBusinessAccount>): Promise<FacebookBusinessAccount | undefined> {
    const result = await db.update(facebookBusinessAccounts)
      .set({ ...updates, updatedAt: new Date() })
      .where(and(
        eq(facebookBusinessAccounts.id, id),
        eq(facebookBusinessAccounts.dealershipId, dealershipId)
      ))
      .returning();
    return result[0];
  ...
  // ====== LISTINGS ======
  async getActiveListingInstanceByVin(dealershipId: number, vin: string): Promise<ListingInstance | undefined> {
    const result = await db.select().from(listingInstances).where(and(
      eq(listingInstances.dealershipId, dealershipId),
      eq(listingInstances.vin, vin),
      eq(listingInstances.isActive, true)
    )).limit(1);
    return result[0];
  }

  async createListingBundle(bundle: InsertListingBundle): Promise<ListingBundle> {
    if (!bundle.dealershipId) {
      throw new Error('dealershipId is required when creating a listing bundle');
    }
    const result = await db.insert(listingBundles).values(bundle).returning();
    return result[0];
  }
  ...
  async updateListingInstance(id: number, dealershipId: number, updates: Partial<InsertListingInstance>): Promise<ListingInstance | undefined> {
    const result = await db.update(listingInstances)
      .set({ ...updates, updatedAt: new Date() })
      .where(and(
        eq(listingInstances.id, id),
        eq(listingInstances.dealershipId, dealershipId)
      ))
      .returning();
    return result[0];
