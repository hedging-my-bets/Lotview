...
export class FacebookService {
  ...
  getBusinessAuthUrl(state: string, dealershipConfig?: { facebookAppId?: string | null; facebookAppSecret?: string | null }): string {
    const config = this.getConfig(dealershipConfig);
    const params = new URLSearchParams({
      client_id: config.appId,
      redirect_uri: config.redirectUri,
  ...
  async getUserBusinesses(accessToken: string): Promise<Array<{ id: string; name: string }>> {
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.error?.message || 'Failed to get user businesses');
    }
  ...
  async postMarketplaceListing(accessToken: string, listing: MarketplaceListingDraft): Promise<{ postId: string }> {
    const formData = new FormData();
    formData.append('access_token', accessToken);
    formData.append('title', listing.title);
    formData.append('description', listing.description);
    formData.append('price', listing.price?.toString() || '0');
    formData.append('currency', listing.currency || 'CAD');
    formData.append('availability', listing.availability || 'in stock');
  ...
  async postToMarketplace(
    ...
    const listing = buildMarketplaceListingDraft(vehicle, template);
    return this.postMarketplaceListing(accessToken, listing);
  }
