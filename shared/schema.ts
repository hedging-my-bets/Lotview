...
// Dealer groups - top-level org with multiple rooftops
...
export const insertDealerGroupSchema = createInsertSchema(dealerGroups).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export type InsertDealerGroup = z.infer<typeof insertDealerGroupSchema>;
...
export const dealerships = pgTable("dealerships", {
  id: serial("id").primaryKey(),
  dealerGroupId: integer("dealer_group_id").references(() => dealerGroups.id, { onDelete: 'set null' }),
  name: text("name").notNull(), // e.g., "Olympic Auto Group"
  ...
  pageDmLink: text("page_dm_link"), // Default Page DM link for Marketplace CTA
  isActive: boolean("is_active").notNull().default(true),
  createdAt: timestamp("created_at").defaultNow().notNull(),
  updatedAt: timestamp("updated_at").defaultNow().notNull(),
});
...
// Rooftops (stores) - sub-entities under a dealer group
...
export const insertRooftopSchema = createInsertSchema(rooftops).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export type InsertRooftop = z.infer<typeof insertRooftopSchema>;
export type Rooftop = typeof rooftops.$inferSelect;

// Rooftop memberships - RBAC for posting rights
export const rooftopMemberships = pgTable("rooftop_memberships", {
  id: serial("id").primaryKey(),
  dealershipId: integer("dealership_id").notNull().references(() => dealerships.id, { onDelete: 'cascade' }),
  rooftopId: integer("rooftop_id").notNull().references(() => rooftops.id, { onDelete: 'cascade' }),
...
export const insertRooftopMembershipSchema = createInsertSchema(rooftopMemberships).omit({
  id: true,
  createdAt: true,
});

export type InsertRooftopMembership = z.infer<typeof insertRooftopMembershipSchema>;
export type RooftopMembership = typeof rooftopMemberships.$inferSelect;
...
// Facebook Business accounts (system user tokens + business metadata)
export const facebookBusinessAccounts = pgTable("facebook_business_accounts", {
  id: serial("id").primaryKey(),
  dealershipId: integer("dealership_id").notNull().references(() => dealerships.id, { onDelete: 'cascade' }),
...
export const insertFacebookBusinessAccountSchema = createInsertSchema(facebookBusinessAccounts).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export type InsertFacebookBusinessAccount = z.infer<typeof insertFacebookBusinessAccountSchema>;
export type FacebookBusinessAccount = typeof facebookBusinessAccounts.$inferSelect;
...
// Listing bundle - Marketplace-ready content per vehicle
export const listingBundles = pgTable("listing_bundles", {
  id: serial("id").primaryKey(),
  dealershipId: integer("dealership_id").notNull().references(() => dealerships.id, { onDelete: 'cascade' }),
  rooftopId: integer("rooftop_id").references(() => rooftops.id, { onDelete: 'set null' }),
  ...
  imageUrls: text("image_urls").array(), // Ordered image URLs
...
export const insertListingBundleSchema = createInsertSchema(listingBundles).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export type InsertListingBundle = z.infer<typeof insertListingBundleSchema>;
export type ListingBundle = typeof listingBundles.$inferSelect;

// Listing instance - One posted listing record
export const listingInstances = pgTable("listing_instances", {
  id: serial("id").primaryKey(),
  dealershipId: integer("dealership_id").notNull().references(() => dealerships.id, { onDelete: 'cascade' }),
  dealerGroupId: integer("dealer_group_id").references(() => dealerGroups.id, { onDelete: 'set null' }),
  rooftopId: integer("rooftop_id").references(() => rooftops.id, { onDelete: 'set null' }),
  vehicleId: integer("vehicle_id").notNull().references(() => vehicles.id, { onDelete: 'cascade' }),
  bundleId: integer("bundle_id").references(() => listingBundles.id, { onDelete: 'set null' }),
  vin: text("vin").notNull(),
  channel: text("channel").notNull().default('facebook_marketplace'), // 'facebook_marketplace', 'facebook_page', 'instagram'
  posterType: text("poster_type").notNull().default('personal'), // 'personal', 'page'
  listingUrl: text("listing_url"),
  externalListingId: text("external_listing_id"),
  status: text("status").notNull().default('draft'), // 'draft', 'queued', 'posting', 'posted', 'removed', 'failed'
  isActive: boolean("is_active").notNull().default(true),
...
export const insertListingInstanceSchema = createInsertSchema(listingInstances).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

export type InsertListingInstance = z.infer<typeof insertListingInstanceSchema>;
export type ListingInstance = typeof listingInstances.$inferSelect;
...
export const messengerConversations = pgTable("messenger_conversations", {
  id: serial("id").primaryKey(),
  dealershipId: integer("dealership_id").notNull().references(() => dealerships.id, { onDelete: 'cascade' }),
  facebookAccountId: integer("facebook_account_id").references(() => facebookAccounts.id, { onDelete: 'cascade' }),
  rooftopId: integer("rooftop_id").references(() => rooftops.id, { onDelete: 'set null' }),
  pageId: text("page_id").notNull(), // Facebook Page ID
